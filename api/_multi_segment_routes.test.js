...
test('#540: every known multi-segment /api route has a vercel.json rewrite', () => {
  ...
  for (const p of multiSegmentRoutes) {
    ...
    // Carriers must be reachable: either a single-segment path (the only
    // shape the platform router hands to the catch-all function — verified
    // live, #540) or a route-table spelling whose dotted form is itself
    // rewritten (agent/*.json <-> agent/<name>, reconciled with the
    // dotted-routes contract).
    const destPath = dest.split('?')[0];
    const seg = destPath.replace(/^\/api\//, '');
    const isSingle = seg && !seg.includes('/');
    const bare = destPath.replace(/^\/api/, '');
    const isRouteSpelling = ROUTES.some((r) => r.path === bare || r.path.replace(/\.(json|xml)$/, '') === bare);
    assert.ok(
      isSingle || isRouteSpelling,
      `/api${p} -> ${dest}: carrier is neither single-segment nor a routed spelling`
    );
  }
});
