...
export default async function handler(req, res) {
  ...
  try {
    ...
    // canonicalApiPath() then restores the canonical dotted path for
    // extensionless spellings of dotted routes (#548 #380 #468).
    const clean = canonicalApiPath(
      pathname.replace(/^\/v1\//, '/').replace(/\/+$/, '') || '/'
    );
