 ...
 */
export function parsePagination(q, { defaultLimit, maxLimit, scope = null, keyShape = 'pair' } = {}) {
  ...
  if (raw != null && raw !== '') {
    ...
    // 400. Endpoints sorting on the shared [number, string] total order
    // (metric desc + unique id asc) enforce that pair shape; endpoints with
    // their own single-value keyset (e.g. /api/runs on the runId string)
    // declare `keyShape: 'string'` and get the equivalent strictness for it.
    const keyOk = keyShape === 'string'
      ? typeof cursor === 'string' && cursor.length > 0
      : cursor != null && isValidSortKey(cursor);
    if (!keyOk) throw new InvalidCursorError();
  }
  return { limit, cursor };
}
