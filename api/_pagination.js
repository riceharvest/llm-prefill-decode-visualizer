...
/**
 * Shared list envelope (#951): ONE shape for every list endpoint.
 *
 *   { ..., total, items[], [has_more, next_cursor] }
 *
 * - `items` is THE collection key on every list endpoint (matches the
 *   cursor-pagination contract documented at the top of this file);
 * - `total` is exactly one top-level count: the size of the full filtered
 *   collection (NOT the returned page);
 * - pass `pagination` ({ has_more, next_cursor }) when the endpoint pages;
 * - endpoints that historically shipped another collection key (`runs`,
 *   `results`, `groups`, `snapshots`) keep that key as an alias via
 *   `aliases` during the deprecation window (additive, no version bump).
 *
 * Everything else (`description`, `snapshot`, caveats, ...) rides through
 * as-is so handlers read as a plain body literal.
 */
