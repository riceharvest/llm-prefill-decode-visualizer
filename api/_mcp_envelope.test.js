...
test('JSON-RPC errors keep the envelope: code + message, echoed id', async () => {
  // resources/list used to be the canonical unimplemented method here; since
  // #794 it is served for real, so an actually-unsupported method probes the
  // same error path.
  const { status, body } = await rpc({ jsonrpc: '2.0', id: 13, method: 'prompts/list' });
