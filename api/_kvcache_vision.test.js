...
test('#643 legacy kvCache call has NO vision block (byte-shape preserved)', () => {
  const { status, json } = call({ model: 'kvCache', architecture: 'llama70b', contextLength: '65536' });
...
test('#643 imgRes+imgN extend the KV context and echo the breakdown', () => {
  const { status, json } = call({ model: 'kvCache', architecture: 'llama70b', contextLength: '32768', imgRes: '4k', imgN: '3' });
  assert.equal(status, 200);
...
test('#643 explicit visionTokens spelling works and mints a distinct calc id', () => {
  const base = call({ model: 'kvCache', contextLength: '32768' });
  const withVision = call({ model: 'kvCache', contextLength: '32768', visionTokens: '1000' });
...
test('#643 invalid vision params fail loudly (400 problem, not silent all-text)', () => {
  const bad = call({ model: 'kvCache', imgRes: '8k' });
  assert.equal(bad.status, 400);
  const neg = call({ model: 'kvCache', visionTokens: '-1' });
