...
test('#543: new operations are documented with a camelCase operationId', async () => {
  // Note: per-operation x-examples/x-rate-limit blocks were part of an earlier
  // draft; main stamps rate-limit metadata via ENFORCED_PATHS instead.
  const spec = await fetchSpec();
  for (const p of NEW_PATHS) {
    const op = spec.paths[p]?.get;
    assert.ok(op, `${p} missing GET op`);
    assert.match(op.operationId, /^[a-z][A-Za-z0-9]*$/, `${p}: bad operationId`);
  }
});
