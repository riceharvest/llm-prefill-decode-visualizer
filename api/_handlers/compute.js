...
// omits promptTokens (the shape of every capability-list example), the math
// silently runs on 2× the prompt of its singleTurn comparison call. Surface
// exactly which workload params were defaulted so cross-model comparisons
// aren't apples-to-oranges. Additive: bodies without omitted params unchanged.
...
function computeOneInner(params, dryRun = false) {
  const model = params.model || params.m || '';
...
// Public wrapper: annotates defaulted workload params (#742) on every
// successful single-item computation (standalone calls and batch items alike).
function computeOne(params, dryRun = false) {
  return withAssumedDefaults(params, computeOneInner(params, dryRun));
}
