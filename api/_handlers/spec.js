...
const RUN = {
  ...
  properties: {
    ...
    prefillTokPerSecExact: { type: ['number', 'null'], description: 'Unrounded measured prompt-processing speed (tok/s); null when the source row predates exact capture' },
    decodeTokPerSecExact: { type: ['number', 'null'], description: 'Unrounded measured decode speed (tok/s); null when the source row predates exact capture' },
...
const RUN_LIST_ENVELOPE = {
  ...
  properties: {
    ...
    limit: { type: 'integer', description: 'Effective page size after the 200 hard cap' },
...
const BENCHMARK_GROUP_LIST_ENVELOPE = {
  ...
  properties: {
    ...
    limit: { type: 'integer', description: 'Effective page size after the 200 hard cap (#994 clamp telemetry)' },
    units: { type: 'object', description: 'In-band unit declarations for every aggregate speed (all values tok/s) (#776)' },
...
const BEST_LIST_ENVELOPE = {
  ...
  properties: {
    ...
    requestedScenario: { type: ['string', 'null'], description: 'Echoed ?scenario= id (null when unset)' },
    fitCheck: { type: 'boolean', description: 'Whether a fitCheck was requested via ?fitCheck=true' },
    minDecode: { type: ['number', 'null'], description: 'Echoed minimum decode speed filter (#513)' },
