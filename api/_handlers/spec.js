...
const BEST_LIST_ENVELOPE = {
  ...
  properties: {
    ...
    runsBeforeFit: { type: 'integer', description: 'Comparable runs entering the fitCheck stage, i.e. matchedRuns + excludedRuns (present only with fitCheck)' },
    excludedByMaxVramGb: { type: 'integer', description: 'Runs dropped by ?maxVramGb= for exceeding the cap (present only with ?maxVramGb=)' },
    excludedUnknownVramGb: { type: 'integer', description: 'Runs dropped by ?maxVramGb= because their memory size is unknown — a data gap, not an over-cap verdict (present only with ?maxVramGb=)' },
    maxAgeDays: { type: ['number', 'null'], description: 'Echoed ?max_age= filter (null when unset)' },
    contextBand: { type: ['string', 'null'], enum: ['lt1k', '1k-8k', '8k-32k', '32k+', null], description: 'Echoed ?context_band= filter (null when unset)' },
    caveats: { type: 'array', items: { $ref: '#/components/schemas/Caveat' } },
