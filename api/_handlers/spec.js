...
const BEST_LIST_ENVELOPE = {
  ...
  properties: {
    ...
    runsBeforeFit: { type: 'integer', description: 'Runs present immediately before the fitCheck filter ran (present only with fitCheck); excludedRuns = runsBeforeFit − matchedRuns' },
    excludedUnknownVramGb: { type: 'integer', description: 'Runs dropped by ?maxVramGb whose memory size was unknown (no vramGb/unifiedMemoryGb) rather than over-cap (present only when ?maxVramGb was applied)' },
    maxAgeDays: { type: ['number', 'null'], description: 'Echoed ?max_age= filter (null when unset)' },
    contextBand: { type: ['string', 'null'], enum: ['lt1k', '1k-8k', '8k-32k', '32k+', null], description: 'Echoed ?context_band= filter (null when unset)' },
    caveats: { type: 'array', items: { $ref: '#/components/schemas/Caveat' } },
      ...
      '/api/best': {
        get: {
          ...
          parameters: [
            ...
            { name: 'powerDrawWatts', in: 'query', schema: { type: 'number' }, description: 'cost mode: alias for powerWatts (same spelling /api/compute documents); when both are given powerDrawWatts wins' },
