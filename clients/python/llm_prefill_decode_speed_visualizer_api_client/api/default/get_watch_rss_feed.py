from http import HTTPStatus
from typing import Any, cast
from urllib.parse import quote

import httpx

from ...client import AuthenticatedClient, Client
from ...types import Response, UNSET
from ... import errors

from ...models.get_watch_rss_feed_response_429 import GetWatchRssFeedResponse429
from ...types import UNSET, Unset
from typing import cast



def _get_kwargs(
    *,
    model: str | Unset = UNSET,
    hardware: str | Unset = UNSET,
    quant: str | Unset = UNSET,
    days: int | Unset = 30,

) -> dict[str, Any]:
    

    

    params: dict[str, Any] = {}

    params["model"] = model

    params["hardware"] = hardware

    params["quant"] = quant

    params["days"] = days


    params = {k: v for k, v in params.items() if v is not UNSET and v is not None}


    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/api/watch/rss.xml",
        "params": params,
    }


    return _kwargs



def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Any | GetWatchRssFeedResponse429 | None:
    if response.status_code == 200:
        response_200 = cast(Any, None)
        return response_200

    if response.status_code == 429:
        response_429 = GetWatchRssFeedResponse429.from_dict(response.json())



        return response_429

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Response[Any | GetWatchRssFeedResponse429]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    *,
    client: AuthenticatedClient | Client,
    model: str | Unset = UNSET,
    hardware: str | Unset = UNSET,
    quant: str | Unset = UNSET,
    days: int | Unset = 30,

) -> Response[Any | GetWatchRssFeedResponse429]:
    """ RSS 2.0 feed of community runs for a watched combo (#109)

     Filters mirror GET /api/localmaxxing (model/hardware substring, quant exact). Items are the newest
    matching runs (max 50), each linking to the upstream run. Poll like any feed — no registration
    needed.

    Args:
        model (str | Unset):
        hardware (str | Unset):
        quant (str | Unset):
        days (int | Unset):  Default: 30.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Any | GetWatchRssFeedResponse429]
     """


    kwargs = _get_kwargs(
        model=model,
hardware=hardware,
quant=quant,
days=days,

    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)

def sync(
    *,
    client: AuthenticatedClient | Client,
    model: str | Unset = UNSET,
    hardware: str | Unset = UNSET,
    quant: str | Unset = UNSET,
    days: int | Unset = 30,

) -> Any | GetWatchRssFeedResponse429 | None:
    """ RSS 2.0 feed of community runs for a watched combo (#109)

     Filters mirror GET /api/localmaxxing (model/hardware substring, quant exact). Items are the newest
    matching runs (max 50), each linking to the upstream run. Poll like any feed — no registration
    needed.

    Args:
        model (str | Unset):
        hardware (str | Unset):
        quant (str | Unset):
        days (int | Unset):  Default: 30.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Any | GetWatchRssFeedResponse429
     """


    return sync_detailed(
        client=client,
model=model,
hardware=hardware,
quant=quant,
days=days,

    ).parsed

async def asyncio_detailed(
    *,
    client: AuthenticatedClient | Client,
    model: str | Unset = UNSET,
    hardware: str | Unset = UNSET,
    quant: str | Unset = UNSET,
    days: int | Unset = 30,

) -> Response[Any | GetWatchRssFeedResponse429]:
    """ RSS 2.0 feed of community runs for a watched combo (#109)

     Filters mirror GET /api/localmaxxing (model/hardware substring, quant exact). Items are the newest
    matching runs (max 50), each linking to the upstream run. Poll like any feed — no registration
    needed.

    Args:
        model (str | Unset):
        hardware (str | Unset):
        quant (str | Unset):
        days (int | Unset):  Default: 30.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Any | GetWatchRssFeedResponse429]
     """


    kwargs = _get_kwargs(
        model=model,
hardware=hardware,
quant=quant,
days=days,

    )

    response = await client.get_async_httpx_client().request(
        **kwargs
    )

    return _build_response(client=client, response=response)

async def asyncio(
    *,
    client: AuthenticatedClient | Client,
    model: str | Unset = UNSET,
    hardware: str | Unset = UNSET,
    quant: str | Unset = UNSET,
    days: int | Unset = 30,

) -> Any | GetWatchRssFeedResponse429 | None:
    """ RSS 2.0 feed of community runs for a watched combo (#109)

     Filters mirror GET /api/localmaxxing (model/hardware substring, quant exact). Items are the newest
    matching runs (max 50), each linking to the upstream run. Poll like any feed — no registration
    needed.

    Args:
        model (str | Unset):
        hardware (str | Unset):
        quant (str | Unset):
        days (int | Unset):  Default: 30.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Any | GetWatchRssFeedResponse429
     """


    return (await asyncio_detailed(
        client=client,
model=model,
hardware=hardware,
quant=quant,
days=days,

    )).parsed
