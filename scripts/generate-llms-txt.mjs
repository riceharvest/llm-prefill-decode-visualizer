...
export function renderTabsTail(tabs = TABS) {
  return [
    ...
    '',
    'The human-facing UI is this same site. Deep links carry state via URL params: `?tab=<id>`, `preset=<hardware-id>`, `prompt=`, `output=`, `spec=1&draftK=&acc=`, `autoplay=1` (auto-start the simulation on load — machine-friendly; preserved by share and export links), `title=` (free-text page title a share link carries). Keyboard: Space play/pause, R reset, Ctrl+Z undo, Ctrl+Shift+Z redo, 1–9 and 0 switch tabs, ? opens the shortcuts dialog.',
    '',
    '<!-- tabs-section:start -->',
