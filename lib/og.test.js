...
// #947: explicit ?v= pins a rendering — it participates in the cache key so
// a pinned URL keeps its exact pixels across layout bumps. Absent ?v= must
// leave keys byte-identical to the pre-#947 scheme.
test('parseOgParams carries an optional v pin tag', () => {
  const plain = parseOgParams(sp({ preset: 'h100' }));
  assert.equal(plain.v, undefined);
  const pinned = parseOgParams(sp({ preset: 'h100', v: '2026-08-24' }));
  assert.equal(pinned.v, '2026-08-24');
});

test('cacheKeyFor: ?v= changes the key, absence keeps the legacy key', () => {
  const base = parseOgParams(sp({ preset: 'h100', prefill: '9500', decode: '130' }));
  const pinA = parseOgParams(sp({ preset: 'h100', prefill: '9500', decode: '130', v: 'a' }));
  const pinA2 = parseOgParams(sp({ preset: 'h100', prefill: '9500', decode: '130', v: 'a' }));
  const pinB = parseOgParams(sp({ preset: 'h100', prefill: '9500', decode: '130', v: 'b' }));
  assert.equal(cacheKeyFor(base), cacheKeyFor(parseOgParams(sp({ preset: 'h100', prefill: '9500', decode: '130' }))));
  assert.notEqual(cacheKeyFor(base), cacheKeyFor(pinA));
  assert.equal(cacheKeyFor(pinA), cacheKeyFor(pinA2));
  assert.notEqual(cacheKeyFor(pinA), cacheKeyFor(pinB));
});
