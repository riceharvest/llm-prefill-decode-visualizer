...
import handler, { bitsPerWeight } from '../api/_handlers/sizing.js';
import { kvCache } from '../api/_math.js';
...
test('explicit maxTpotMs=0 / maxTtftSeconds=0 are honored as strict caps, not dropped (#731)', async () => {
  const { json } = await call({ query: { model: 'qwen', maxTpotMs: 0 } });
  assert.equal(json.slo.maxTpotMs, 0); // echoed as set — not silently nulled
  assert.ok(json.recommendations.length > 0);
  for (const rec of json.recommendations) {
    assert.equal(rec.meetsSlo.tpot, false); // no rig achieves 0 ms/token
    assert.equal(rec.meetsSlo.all, false);
  }

  const ttft = await call({ query: { model: 'qwen', maxTtftSeconds: 0 } });
  assert.equal(ttft.json.slo.maxTtftSeconds, 0);
  for (const rec of ttft.json.recommendations) {
    assert.equal(rec.meetsSlo.ttft, false);
    assert.equal(rec.meetsSlo.all, false);
  }
});
...
test('#632: budget cap excludes unknown-memory rigs instead of passing them', async () => {
  // r4 (old-laptop) has vramGb:null. With a cap set it must NOT rank as
  // "under budget"; without a cap it still appears.
  const uncapped = await call({ query: { model: 'qwen' } });
  assert.ok(uncapped.json.budgetCap === undefined, 'no budgetCap key when no cap is requested');
  assert.ok(uncapped.json.recommendations.some(r => r.memoryGb == null));

  const capped = await call({ query: { model: 'qwen', maxVramGb: '200' } });
  assert.deepEqual(capped.json.budgetCap, { maxVramGb: 200, excludedUnknownMemoryRuns: 1 });
  // r3 (256GB) drops as over-budget, r4 (null) drops as unknown-memory.
  assert.equal(capped.json.matchedRuns, 2);
  assert.ok(capped.json.recommendations.every(r => r.memoryGb != null && r.memoryGb <= 200));
});

test('#632: cap that only unknown-memory rigs could pass yields loud 404 with echo', async () => {
  // maxVramGb=4 excludes every known-memory rig; previously r4 (null) would
  // sneak through as an under-budget recommendation.
  const { status, json } = await call({ query: { model: 'qwen', maxVramGb: '4' } });
  assert.equal(status, 404);
  assert.match(json.error, /No comparable benchmark runs/);
  assert.deepEqual(json.budgetCap, { maxVramGb: 4, excludedUnknownMemoryRuns: 1 });
});
