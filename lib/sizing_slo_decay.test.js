...
test('#648 expected.walltimeSeconds is always derivable in the response', async () => {
...
test('#648 ?maxWalltimeSeconds adds the third verdict + margins; failing walltime excludes the rig', async () => {
...
test('#648 latency margins are numeric with the UI convention, null when unevaluated', async () => {
...
test('#636 absent decay param → legacy empty-cache TPOT, no contextScaling block', async () => {
  const { json } = await call({ query: { model: 'qwen' } });
  assert.equal(json.contextScaling, undefined);
  assert.equal(json.recommendations[0].expected.contextScaling, undefined);
});

test('#636 ?halfSpeedContextTokens= decays TPOT via the closed form and feeds SLOs', async () => {
  const ctxHalf = 8192;
...
test('#636 ?ctxHalf alias works identically', async () => {
