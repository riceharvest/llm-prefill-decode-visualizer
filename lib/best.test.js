...
test('by=cost honors the documented powerDrawWatts spelling (#1111)', async () => {
  const viaAlias = await call({ by: 'cost', price: '700', powerDrawWatts: '450' });
  const viaCanonical = await call({ by: 'cost', price: '700', powerWatts: '450' });
  assert.equal(viaAlias.status, 200);
  assert.equal(viaCanonical.status, 200);
  assert.equal(viaAlias.json.results[0].costInputs.powerDrawWatts, 450);
  assert.equal(viaCanonical.json.results[0].costInputs.powerDrawWatts, 450);
});

test('by=cost matches hwClass case-insensitively against power defaults (#1111)', async () => {
  // The live dataset ships UPPERCASE hwClass values; the old exact-match
  // lookup silently fell back to a flat 150 W for every rig.
  const { invalidateCache } = await import('../api/_localmaxxing.js');
  invalidateCache(); // drop rows cached by earlier tests before re-mocking fetch
  globalThis.fetch = async () => ({ ok: true, json: async () => ({ rows: [
    { ...ROWS[0], hardware: { ...ROWS[0].hardware, hwClass: 'DISCRETE_GPU' } },
    { ...ROWS[1], hardware: { ...ROWS[1].hardware, hwClass: 'UNIFIED' } }
  ] }) });
  const { json } = await call({ by: 'cost' });
  const byKey = Object.fromEntries(json.results.map(r => [r.hardwareKey, r]));
  assert.equal(byKey.riga.costInputs.powerDrawWatts, 300); // discrete_gpu default
  assert.equal(byKey.rigb.costInputs.powerDrawWatts, 60); // unified default
});
