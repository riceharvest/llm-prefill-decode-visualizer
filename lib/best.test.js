...
<<<<<<< main
test('#631: by=cost honors ?scenario= workload shape instead of silent chat defaults', async () => {
  // Default (no scenario): chat shape 2048/512.
  const base = await call({ by: 'cost' });
  assert.equal(base.json.results[0].costInputs.promptTokens, 2048);
  assert.equal(base.json.results[0].costInputs.outputTokens, 512);

  // Scenario preset (rag = 4096/512) now flows into the cost math echo…
  const rag = await call({ by: 'cost', scenario: 'rag' });
  assert.equal(rag.json.workload.source, 'scenario:rag');
  assert.equal(rag.json.results[0].costInputs.promptTokens, 4096);
  assert.equal(rag.json.results[0].costInputs.outputTokens, 512);

  // …and changes the ranking inputs, not just the echo: a prompt-heavy
  // shape must shift effective throughput vs the chat-shape run.
  const chatRow = base.json.results.find(r => r.hardwareKey === 'rigb');
  const ragRow = rag.json.results.find(r => r.hardwareKey === 'rigb');
  assert.ok(
    Math.abs(chatRow.effectiveThroughputTokPerSec - ragRow.effectiveThroughputTokPerSec) > 1e-9,
    'scenario shape should change the priced workload, not only labels'
  );

  // Explicit tokens still win over the scenario preset.
  const explicit = await call({ by: 'cost', scenario: 'rag', promptTokens: '1024', outputTokens: '128' });
  assert.equal(explicit.json.results[0].costInputs.promptTokens, 1024);
  assert.equal(explicit.json.results[0].costInputs.outputTokens, 128);

  // Unknown scenario falls back to chat defaults (no crash, same as walltime).
  const unknown = await call({ by: 'cost', scenario: 'nope' });
  assert.equal(unknown.json.results[0].costInputs.promptTokens, 2048);
=======
// fitCheck tri-state (#713): explicit false must win over contextLength.
test('fitCheck=false disables the VRAM-fit filter even with contextLength>0 (#713)', async () => {
  const off = await call({ by: 'decode', limit: '10', fitCheck: 'false', contextLength: '100000000' });
  assert.equal(off.json.fitCheck, false);
  const plain = await call({ by: 'decode', limit: '10' });
  assert.deepEqual(off.json.results.map(r => r.key ?? r.hardwareKey), plain.json.results.map(r => r.key ?? r.hardwareKey));
});

test('fitCheck accepts the shared vocabulary and echoes the resolved state (#688 #713)', async () => {
  for (const v of ['true', 'TRUE', 'yes', 'on', '1']) {
    const { json } = await call({ by: 'decode', fitCheck: v });
    assert.equal(json.fitCheck, true, v);
  }
  for (const v of ['no', 'off', '0']) {
    const { json } = await call({ by: 'decode', fitCheck: v });
    assert.equal(json.fitCheck, false, v);
  }
});

test('contextLength alone still implies fitCheck; unrecognized value warns (#713 #688)', async () => {
  const implied = await call({ by: 'decode', contextLength: '8192' });
  assert.equal(implied.json.fitCheck, true);
  const bogus = await call({ by: 'decode', fitCheck: 'maybe' });
  assert.equal(bogus.json.fitCheck, false);
  assert.ok(bogus.json.warnings.some(w => w.includes("fitCheck='maybe'")), JSON.stringify(bogus.json.warnings));
>>>>>>> base
});
