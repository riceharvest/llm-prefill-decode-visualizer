...
const ROWS = [
  ...
  // RigC: dominated on both axes (so it never wins an existing ranking) but
  // carries hwClass in the UPPERCASE spelling live upstream data uses (#482) —
  // exercises the per-class watt fallback in by=cost (#1111).
  {
    id: 'c1', batchSize: 1,
    tokSPrefill: 1500, tokSOut: 60,
    model: { hfId: 'meta-llama/Llama-3.1-8B-Instruct', displayName: 'Llama 3.1 8B', params: 8 },
    hardwareGroupKey: 'rigc', hardwareGroupLabel: 'Rig C',
    hardware: { hwClass: 'UNIFIED', gpuName: 'TestChip C', gpuCount: 1, vramGb: 36 },
    engine: { engineName: 'llama.cpp', quantization: 'q4_k_m' }
  }
];
...
test('by=cost per-class watt fallback works with UPPERCASE wire hwClass (#1111)', async () => {
  // Live upstream sends hwClass like 'UNIFIED'; before the fix the lookup
  // DEFAULT_POWER_WATTS[hwClass] missed and every rig fell back to a flat 150W.
  const { json } = await call({ by: 'cost' });
  const rigc = json.results.find(r => r.hardwareKey === 'rigc');
  assert.ok(rigc, 'rigc should be ranked');
  assert.equal(rigc.hwClass, 'UNIFIED');
  assert.equal(rigc.costInputs.powerDrawWatts, 60, 'unified rigs should use the 60W per-class estimate, not the flat 150W fallback');
});

test('by=cost accepts compute\'s powerDrawWatts spelling as an alias (#1111)', async () => {
  const viaAlias = await call({ by: 'cost', powerDrawWatts: '777' });
  for (const r of viaAlias.json.results) {
    assert.equal(r.costInputs.powerDrawWatts, 777, `powerDrawWatts alias ignored on ${r.hardwareKey}`);
  }
  const viaCanonical = await call({ by: 'cost', powerWatts: '888' });
  for (const r of viaCanonical.json.results) {
    assert.equal(r.costInputs.powerDrawWatts, 888);
  }
});
