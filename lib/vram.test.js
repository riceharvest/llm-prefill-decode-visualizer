...
test('missing hfId → 400 with guidance; unknown repo → 404 passthrough', async () => {
  ...
  assert.match(missing.json.detail, /hfId/); // problem+json contract (#539)
  assert.equal(missing.json.code, 'INVALID_PARAMS');

  installFetch({ config: null });
  const notFound = await call({ query: { hfId: 'org/does-not-exist-xyz' } });
  assert.equal(notFound.status, 404);
  assert.match(notFound.json.detail, /hfId|Hugging Face/i);
});
