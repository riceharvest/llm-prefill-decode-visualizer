...
test('capability list documents the sanity warnings', () => {
  ...
  assert.equal(json.sanity.codes.length, 5); // 3 roofline + 2 cost-mode zero-default codes (#736)
...
test('cost mode carries a warnings array; zero-default price/power flagged (#736 #798)', () => {
  // Fully specified inputs → documented non-blocking array, empty.
  const explicit = call({
    query: { model: 'cost', hardwarePriceUsd: '2000', powerDrawWatts: '450' }
  });
  assert.equal(explicit.status, 200);
  assert.deepEqual(explicit.json.warnings, []);
  assert.equal(explicit.json.inputs.powerDrawWatts, 450);

  // Defaults left in place → each silent zero default is flagged by code,
  // math untouched (still 200, still computed).
  const defaulted = call({ query: { model: 'cost' } });
  assert.equal(defaulted.status, 200);
  const codes = defaulted.json.warnings.map(w => w.code).sort();
  assert.deepEqual(codes, ['cost_hardware_price_unspecified', 'cost_power_draw_unspecified']);
  assert.equal(defaulted.json.costUsdPerMillionTokens, 0);
  for (const w of defaulted.json.warnings) {
    assert.equal(typeof w.message, 'string');
    assert.ok(w.message.length > 0);
  }

  // ?price= alias counts as providing the hardware price (#550 family).
  const alias = call({ query: { model: 'cost', price: '700', powerDrawWatts: '300' } });
  assert.deepEqual(alias.json.warnings, []);
});

test('cost dry_run echo is unchanged by the warnings addition (#736)', () => {
  const { status, json } = call({ query: { model: 'cost', dry_run: 'true' } });
  assert.equal(status, 200);
  assert.equal(json.dry_run, true);
  assert.ok(!('warnings' in json), 'dry_run echoes parsed params only');
});
