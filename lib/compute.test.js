...
test('#1020: flagged mints the same id with or without explicit defaults (resolved-inputs hashing)', () => {
  const bare = call({ query: { model: 'flagged', flags: 'flash-attn' } });
  const full = call({
    query: {
      model: 'flagged', flags: 'flash-attn',
      prefillSpeed: '3800', decodeSpeed: '105', promptTokens: '2048', outputTokens: '512'
    }
  });
  assert.equal(bare.status, 200);
  assert.match(bare.json.id, /^calc_[0-9a-f]{12}$/);
  assert.equal(full.json.id, bare.json.id,
    'explicit defaults must not change the flagged calc id');
  // Same computation via POST body must cite the same id too.
  const post = call({ method: 'POST', body: { model: 'flagged', flags: 'flash-attn' } });
  assert.equal(post.json.id, bare.json.id);
});

test('#1020: cost resolves documented aliases before hashing (price === hardwarePriceUsd)', () => {
  const canonical = call({ query: { model: 'cost', hardwarePriceUsd: '2000' } });
  const alias = call({ query: { model: 'cost', price: '2000' } });
  const canonicalRate = call({ query: { model: 'cost', electricityRatePerKwh: '0.2' } });
  const aliasRate = call({ query: { model: 'cost', electricityRate: '0.2' } });
  assert.equal(alias.json.id, canonical.json.id);
  assert.equal(aliasRate.json.id, canonicalRate.json.id);
  // The computed result is unchanged — only the citation-id stability is fixed.
  // (rate_limit.remaining ticks per call, so exclude it from the shape diff.)
  const strip = j => ({ ...j, id: undefined, rate_limit: undefined });
  assert.deepEqual(strip(alias.json), strip(canonical.json));
});

test('#1020: flagged/cost dry_run echoes the SAME resolved id as the real call', () => {
  for (const query of [
    { model: 'flagged', flags: 'flash-attn,kv-q8' },
    { model: 'cost', hardwarePriceUsd: '2000', powerDrawWatts: '450' }
  ]) {
    const real = call({ query });
    const dry = call({ query: { ...query, dry_run: 'true' } });
    assert.equal(dry.status, 200);
    assert.equal(dry.json.dry_run, true);
    assert.equal(dry.json.id, real.json.id, JSON.stringify(query));
  }
});

test('#1020: batch items for flagged/cost carry the same id as standalone calls', () => {
  const item = { model: 'cost', hardwarePriceUsd: 2000 };
  const standalone = call({ query: item });
  const batch = call({ method: 'POST', body: { batch: [item] } });
  assert.equal(batch.json.results[0].ok, true);
  assert.equal(batch.json.results[0].result.id, standalone.json.id);
});
