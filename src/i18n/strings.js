...
export const locales = {
  en: {
    ...
    // Embeddable chart snippets (#104) — copy-paste-ready <img> / iframe
    // snippets rendered from the current chart plus the shared run URL.
    embed: {
      dialogTitle: 'Embed this chart',
      button: 'Embed',
      buttonTooltip: 'Copy a self-contained snippet that renders this chart anywhere',
      generating: 'Rendering chart to PNG…',
      generateError: 'Could not render the chart — try again.',
      htmlLabel: 'Self-contained image (paste anywhere)',
      htmlHint: 'The chart is inlined as a base64 PNG — no hosting or scripts needed, and it links back to this run.',
      markdownLabel: 'Markdown (GitHub READMEs, forums)',
      markdownHint: 'Same inline PNG as a linked markdown image.',
      iframeLabel: 'Live read-only embed',
      iframeHint: 'A small iframe of this exact run URL — live view, no controls beyond scrolling.',
      downloadSvg: 'SVG',
      downloadSvgTooltip: 'Download this chart as an SVG file',
      copy: 'Copy',
      copyTooltip: 'Copy snippet to clipboard',
      close: 'Close',
      defaultAlt: 'LLM prefill/decode inference chart',
      attributionNote: 'Snippets link back to the shared run URL so readers can reproduce your exact configuration.'
    },
    ...
    tabs: {
      ...
      batching: { label: 'Continuous Batching', hint: 'BATCH / ITL' },
    ...
    singleTurn: {
      ...
      ctxScaling: 'Context Scaling:',
      ctxEffectiveTag: 'avg {speed} tok/s over this run',
      ctxHalfLabel: 'Speed-halving context (C½)',
      ctxHalfAria: 'Context depth at which decode speed has halved',
      ctxChartLabel: 'Instantaneous decode speed vs output depth',
      ctxChartAria: 'Decode speed decay curve as the KV cache fills',
      ctxProbeAria: 'Drag to inspect decode speed at a given output depth',
      ctxProbeReadout: 'at +{generated} generated tok · cache ≈ {cache} tok · {speed} tok/s ({pct}% of base)',
      ctxHint: 'Bandwidth model: every decode step re-reads the weights plus the whole KV cache, so time per token grows linearly with cache depth. C½ is the depth where speed has halved — walltime, TPOT and the simulation clock all follow this curve.',
      ...
      itlJitterLabel: 'ITL Jitter:',
      itlVariance: 'Per-token latency variance (CV)',
      itlVarianceAria: 'Coefficient of variation of per-token inter-token latency',
      itlHint: 'Each token draws its own latency from a mean-preserving lognormal (σ² = ln(1+CV²)): average TPOT stays put while the tail grows. Percentiles are table stakes — batched neighbours, preemption and cache misses all live in that p99.',
      itlDistributionLabel: 'ITL distribution · per-token latency',
      itlHistogramAria: 'Histogram of simulated per-token latencies with p50, p95 and p99 markers',
      itlStreamHint: 'Decode now follows the drawn schedule token by token — watch the stream stutter through the p95+ spikes instead of gliding at a constant rate.',
    ...
    batching: {
      paramsPanelAria: 'Continuous batching parameters',
      paramsPanelTitle: 'Continuous Batching & Chunked Prefill Parameters',
      chunkedPrefill: 'Chunked Prefill',
      chunkOff: 'OFF (whole prompt per step)',
      chunkOn: '{size} tok / step',
      chunkAria: 'Chunked prefill size in tokens per engine step',
      chunkValueAria: 'Chunk size value',
      chunkHint: 'Caps how many prompt tokens a single engine step may ingest. Small chunks keep decode steps short (stable ITL); OFF lets one prefill monopolize the step.',
      concurrentRequests: 'Concurrent Requests',
      requestsUnit: 'requests',
      requestsAria: 'Number of client requests in the workload',
      requestsValueAria: 'Concurrent requests value',
      meanPrompt: 'Mean Prompt Tokens',
      meanPromptAria: 'Mean prompt tokens per request',
      meanPromptValueAria: 'Mean prompt tokens value',
      meanOutput: 'Mean Output Tokens',
      meanOutputAria: 'Mean output tokens per request',
      meanOutputValueAria: 'Mean output tokens value',
      maxBatch: 'Max Batch Size',
      maxBatchUnit: 'seqs',
      maxBatchAria: 'Maximum number of running sequences in the batch',
      maxBatchValueAria: 'Max batch size value',
      arrivalInterval: 'Arrival Interval',
      arrivalUnit: 'ms',
      arrivalAria: 'Average milliseconds between request arrivals',
      arrivalValueAria: 'Arrival interval value',
      simStageAria: 'Batch timeline simulation',
      continuousBatchingTag: 'CONTINUOUS BATCHING',
      simulateTag: 'Simulate Batch',
      resetTag: 'Reset Batch',
      statusRunning: 'Step {step} · {count} seqs in batch',
      statusIdle: 'Run the simulation to watch requests join and leave the running batch',
      statusDone: 'All requests completed',
      timelineLabel: 'Batch timeline · requests joining & leaving',
      legendQueue: 'Queued',
      legendPrefill: 'Prefill',
      legendDecode: 'Decode',
      occupancyLabel: 'Batch occupancy · sequences per engine step',
      itlLabel: 'Inter-token latency · request {id}',
      itlSub: 'each bar = gap between two decoded tokens; spikes are steps stretched by prefill chunks',
      currentBatchLabel: 'Running batch · step {step}',
      seqProgress: '{done} / {total} tok',
      metricMakespan: 'Makespan (all done)',
      metricMakespanSub: 'First arrival → last finish',
      metricAvgTtft: 'Avg TTFT',
      metricAvgTtftSub: 'Arrival → first token',
      metricMaxItl: 'Worst ITL',
      metricMaxItlSub: 'Largest gap between tokens',
      metricThroughput: 'Decode throughput',
      metricThroughputSub: '{tokens} output tokens total',
      metricOccupancy: 'Batch occupancy',
      metricOccupancySub: 'of max {max} slots',
      staticBannerPrefix: 'Continuous vs static batching:',
      staticBannerBody: 'makespan reduced from {without} (static cohorts) to {with}',
      staticSavedTag: 'saved {time} ({pct}%)',
      chunkOffPrefix: 'Chunked prefill disabled:',
      chunkOffBody: 'each full prompt prefill monopolizes an engine step — every decoding sequence stalls behind it. Watch the ITL chart spike on prefill steps.',
      rowTooltipQueue: 'R{id} queued {time} waiting for a batch slot',
      rowTooltipPrefill: 'R{id} prefill chunk · {tokens} tok',
      rowTooltipDecode: 'R{id} decoding · 1 tok / step',
      selectRequestHint: 'click a row to inspect its inter-token latency'
    },
    ...
    kvCache: {
      ...
      ledgerAria: 'Full VRAM budget ledger',
      ledgerTitle: 'VRAM Budget Ledger',
      ledgerIntro: "Raw KV cache is only part of the story. This ledger adds model weights at the chosen precision, framework overhead for activations / CUDA graphs / allocator slack (vLLM and llama.cpp both reserve roughly 10–20%), and a safety headroom reserve — then judges the total against a specific GPU's VRAM so OOM shows up here, not at deploy time.",
      precisionLabel: 'Precision (weights & KV)',
      gpuBudgetLabel: 'Target GPU',
      gpuBudgetAria: 'Target GPU VRAM in gigabytes',
      ledgerUtilization: '{pct}% of {vram} GB',
      ledgerFreeAfterReserve: '{gb} GB free after {reserve} GB headroom reserve',
      ledgerOverReserve: 'over the {reserve} GB safety reserve',
      verdictPassDetail: 'leaves the safety reserve intact',
      verdictWarnDetail: 'fits only by eating into the safety reserve — expect OOM under load spikes',
      verdictFailDetail: 'exceeds physical VRAM — reduce precision, context, batch size or move to a bigger GPU',
      perGpuVerdicts: 'Verdict per GPU at current settings',
    },

    multiGpu: {
      panelAria: 'Multi-GPU split planner',
      panelTitle: 'Multi-GPU Split Planner · Tensor & Pipeline Parallel',
      intro: "Pick a 2× or 4× GPU layout and see how model weights and the KV cache above split across the cards, what the interconnect costs you in decode speed, and whether it fits at all. Tensor parallel shards every layer across GPUs and pays an all-reduce on the bus each decode step; pipeline parallel stacks whole layers per GPU and pays a pipeline bubble instead.",
      layoutLabel: 'GPU Count',
      gpuUnit: 'GPUs',
      parallelismLabel: 'Parallelism Mode',
      modeTp: 'Tensor Parallel',
      modePp: 'Pipeline Parallel',
      tpHint: 'weights + KV sharded per layer · all-reduce every decode step',
      ppHint: 'whole layers per GPU · boundary transfers + bubble',
      interconnectLabel: 'Interconnect',
      busPcie: 'PCIe',
      busNvlink: 'NVLink',
      cardLabel: 'Card VRAM',
      weightPrecisionLabel: 'Weight Quant',
      wprecFp16: 'FP16 · 2B',
      wprecFp8: 'FP8 · 1B',
      wprecInt4: 'INT4 · 0.5B',
      metricWeightsPerGpu: 'Weights / GPU',
      metricKvPerGpu: 'KV cache / GPU',
      kvShardedSuffix: 'sharded ÷ {n}',
      kvReplicatedSuffix: 'replicated ×{n}',
      metricVramPerGpu: 'Total needed / GPU',
      metricDecodePenalty: 'Decode speed impact',
      penaltySub: '{base} tok/s single-GPU equivalent → {pct}% {bus} sync loss → {effective} tok/s',
      metricFit: 'Fit verdict ({card})',
      fitYes: 'Fits · {headroom} free',
      fitNo: "Doesn't fit · over by {over}",
      warnDoesNotFit: "{needed} per GPU exceeds {usable} usable on this card ({vram} × 95%). Pick a bigger card, lower the weight quant, or cut context/batch.",
      warnKvReplicated: "Only {kvHeads} KV heads for {n} GPUs in tensor parallel — there is nothing to split, so every card holds a full copy of the KV cache. Pipeline parallel would shard it by layer instead.",
      warnSingleCardFaster: "This whole workload fits on a single {card}. Over PCIe, tensor parallel pays ~10% decode loss to all-reduce every step — one bigger card is both simpler AND faster than two small ones. Only split when no single card can host it.",
      footnote: "Heuristics: PCIe all-reduce ≈ −10% decode for TP (two all-reduce rounds cross the ~64 GB/s bus each step; community-measured deltas cluster at 5–15%), NVLink ≈ −3% (~900 GB/s mostly hides the sync), PP ≈ −2% flat from the pipeline bubble. Usable VRAM = 95% of the card minus ~1.5 GB CUDA/activation overhead per GPU."
    },

    quant: {
      panelAria: 'Quantization tradeoff matrix',
      panelTitle: 'Quantization Tradeoff Matrix · Measured tok/s per Quant',
      intro: "One model family, every community-measured quantization side by side: FP16/FP8 down to GGUF Q4_K_M and below. Click any row to load that quant's best measured speeds into the simulator — the fastest way to answer \"can I drop to Q4 and still hit my tok/s on this card?\".",
      familyLabel: 'Model family',
      familyPlaceholder: 'e.g. llama-3-1-8b or qwen3-6-27b',
      familyAria: 'Model family for the quantization matrix',
      loadTitle: 'Load quantization rows for this family',
      loadAria: 'Load quantization rows for this model family',
      runsCountSuffix: 'runs',
      loading: 'Querying measured quants for this family…',
      errorPrefix: "Couldn't reach /api/benchmarks:",
      noRuns: 'No community-measured quantizations found for this family. Try a shorter family key (e.g. llama-3-1-8b) — repo/quant variants collapse into it.',
      banner: '{family}: {rows} measured quant{rowsPlural} across {runs} single-stream runs. Rows ranked highest-fidelity first.',
      thQuant: 'Quant',
      thBits: 'Bits/weight',
      thRigs: 'Rigs × runs',
      thMedianDecode: 'Median decode (95% CI)',
      thBestMeasured: 'Best measured run',
      thWeightsVram: 'Weights VRAM (est.)',
      thQuality: 'Quality proxy',
      thLoad: 'Load',
      mixedEnginesTitle: 'This row mixes engine builds across rigs — treat the delta with caution',
      vramTitle: 'Estimated weights-only VRAM: parameter count × bits-per-weight ÷ 8. KV cache, activations and framework overhead are excluded.',
      qualityNearLossless: 'Near-lossless vs full precision',
      qualityMinor: 'Minor quality degradation',
      qualitySweetSpot: 'Usual speed/quality sweet spot',
      qualityNoticeable: 'Noticeable quality loss — verify on your task',
      loadIntoSim: 'Load into sim',
      loaded: 'Loaded ✓',
      legend: 'Medians are outlier-resistant per hardware×quant cohort; rigs × runs shows sample size; * = mixed engine builds in the row.',
      viewSource: 'View best run ↗',
      appliedBanner: '{quant} loaded into the simulator at its best measured prefill/decode speeds.',
      agentsNote: 'Agents: the same matrix is available as JSON —',
      estimatesNote: 'Bits-per-weight, weights VRAM and quality notes are clearly-labeled estimates from the quant tag, not benchmark output.'
    },
    ...
    slo: {
      panelAria: 'SLO latency budgets',
      panelTitle: 'SLO budgets',
      toggleTooltip: 'Set TTFT / TPOT / walltime budgets checked against every simulation',
      panelHint: 'Green/red pass-fail badges with margin % appear on every simulation result',
      enabledCount: '{count} active',
      disabled: 'off',
      off: 'off',
      ttftLabel: 'TTFT budget',
      tpotLabel: 'TPOT budget',
      walltimeLabel: 'Walltime budget',
      shortTtft: 'TTFT',
      shortTpot: 'TPOT',
      shortWalltime: 'Walltime',
      inputAria: '{metric} in milliseconds or seconds per the unit shown',
      badgePass: '{label} ✓ {pct}',
      badgeFail: '{label} ✗ {pct} over',
      badgePassTooltip: '{label} within budget ({budget} budgeted) — margin shown as % headroom',
      badgeFailTooltip: '{label} exceeded the {budget} budget — overrun shown as % over',
      marginHint: 'Margin = (budget − actual) ÷ budget. Positive = headroom left, negative = over budget. Clear a field to disable that check. Budgets are saved in your browser.',
      reset: 'Defaults',
      resetTooltip: 'Restore the default budgets (TTFT 500 ms · TPOT 50 ms · walltime 10 s)',
      agenticOffender: '⚠ Turn {turn} blows the budget: {detail}',
      agenticOffenderSuffix: 'Turns {turns} exceed at least one SLO budget.',
      agenticAllPass: '✓ Every turn meets the SLO budgets',
      waterfallBadgeTooltip: 'Turn {turn} vs budget: {detail}'
    },
