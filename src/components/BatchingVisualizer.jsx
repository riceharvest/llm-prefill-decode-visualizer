                ...
                >
                  ...
                  <div style={{ flex: 1, height: '20px', background: 'var(--bg-raised)', borderRadius: 'var(--radius-sm)', position: 'relative', overflow: 'hidden', minWidth: 0 }}>
                    ...
                    {rowSegments.get(req.id)?.length > 0 && (
                      <div
                        role="img"
                        aria-label={queueSegmentLabel(req, rowSegments.get(req.id)[0].tStart - req.arrivalTime)}
                        style={{
                    ...
                    {rowSegments.get(req.id)?.map((seg, i) => (
                      <div
                        key={i}
                        role="img"
                        aria-label={seg.kind === 'prefill' ? prefillSegmentLabel(seg) : decodeSegmentLabel(req, seg)}
                        style={{
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '1px', height: '44px' }} role="img" aria-label={occupancyChartLabel(occupancyBars)}>
            {occupancyBars.map((size, i) => (
              <div
                key={i}
                role="img"
                aria-label={occupancyBarLabel(size, i)}
                style={{
                  flex: 1,
                  minWidth: 0,
              ...
              {selectedRequest.itls.map((itl, i) => {
                ...
                return (
                  <div
                    key={i}
                    role="img"
                    aria-label={itlBarLabel(itl, i, spike)}
                    style={{
                      flex: 1,
                      minWidth: 0,
