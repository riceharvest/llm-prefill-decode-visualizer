...
export default function BatchingVisualizer({
  ...
  // Issue #392 (+#389's DOM-bounds half): the paired number inputs used to
  // pass any typed value straight into state and the URL with no feedback —
  // ?breqs=-5 / breqs=0 hit share links unvalidated, and above-max values
  // gave the range thumb an undiscoverable ceiling. Clamp on commit to each
  // slider's own range and mirror min/max onto the inputs so agents can read
  // the bounds from the DOM. (URL-parse-side clamping of crafted links is a
  // separate layer; this covers interactive input.)
  const commitClampedNumber = (setter, min, max) => (e) => {
    const n = Number(e.target.value);
    if (!Number.isFinite(n)) return;
    setter(Math.min(max, Math.max(min, n)));
    handleReset();
  };
    ...
    >
          ...
          <div className="panel-inset field">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <input type="range" min="2" max="48" step="1" value={numRequests}
                ...
                onChange={(e) => { commitClampedNumber(setNumRequests, 2, 48); handleReset(); }}
                style={{ flex: 1 }} />
          ...
          <div className="panel-inset field">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <input type="range" min="128" max="32768" step="8" value={meanPromptTokens}
                ...
                onChange={(e) => { commitClampedNumber(setMeanPromptTokens, 128, 32768); handleReset(); }}
                style={{ flex: 1 }} />
          ...
          <div className="panel-inset field">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <input type="range" min="32" max="4096" step="32" value={meanOutputTokens}
                ...
                onChange={(e) => { commitClampedNumber(setMeanOutputTokens, 32, 4096); handleReset(); }}
                style={{ flex: 1 }} />
          ...
          <div className="panel-inset field">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <input type="range" min="1" max="32" step="1" value={maxBatchSize}
                ...
                onChange={(e) => { commitClampedNumber(setMaxBatchSize, 1, 32); handleReset(); }}
                style={{ flex: 1 }} />
          ...
          <div className="panel-inset field">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <input type="range" min="0" max="2000" step="10" value={arrivalIntervalMs}
                ...
                onChange={(e) => { commitClampedNumber(setArrivalIntervalMs, 0, 2000); handleReset(); }}
                style={{ flex: 1 }} />
      ...
      >
        ...
        <div className="panel-inset" style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '1px', height: '44px' }} role="img" aria-label={occupancyChartLabel(occupancyBars)}>
        ...
        {selectedRequest && selectedRequest.itls.length > 0 && (
          <div className="panel-inset">
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: '1px', height: '56px' }} role="img" aria-label={itlChartLabel(selectedRequest.itls)}>
