...
// NOTE: consumeAutoplay carries module-level "consumed" state, so this must be
// the only autoplay consumer in the process — run its scenario as one test.
test('consumeAutoplay consumes ?autoplay=1 exactly once per page load (#818 #693)', () => {
  ...
  withSearch('', () => {
    ...
    withSearch('?autoplay=yes', () => assert.equal(consumeAutoplay(), false));
    // demo link → fires once...
    withSearch('?autoplay=1&tab=single', () => assert.equal(consumeAutoplay(), true));
  });
  // ...second consumer in the same page load (tab switch remounts a
  // simulator) must NOT re-fire autoplay...
  withSearch('?autoplay=1&tab=single', () => assert.equal(consumeAutoplay(), false));
  // ...and it stays consumed even after the param is gone.
  withSearch('', () => assert.equal(consumeAutoplay(), false));
});
