...
for (const rel of VIEWS) {
  test(`${rel} completes synchronously before arming rAF (#1079)`, () => {
    ...
    // Views arm playback either via raw rAF or via the shared hidden-tab frame
    // scheduler (#860) — accept both arming forms. The initial arm (the one
    // the hatch must precede) is the LAST arming statement in the effect; the
    // earlier ones are the re-arm sites inside the tick callback itself.
    const armRaw = src.indexOf("animFrameRef.current = requestAnimationFrame(tick)");
    const armSched = src.indexOf("animFrameRef.current = framesRef.current.request(tick)");
    const armIdx = Math.max(armRaw, armSched);
