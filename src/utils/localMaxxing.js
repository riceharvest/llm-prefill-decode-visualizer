...
export function runLabel(run) {
  ...
  return `${hardwareName(run)} · ${engine}${ageTag} · ${formatNum(run.tokSPrefill)} prefill / ${formatNum(run.tokSOut)} decode tok/s`;
}
...
export function toLocalPreset(run, now = new Date()) {
  ...
  return {
    id: `lmx:${run.id}`,
    name: `${hardwareName(run)} (${engine} ${quant} · ${formatNum(run.tokSPrefill)} / ${formatNum(run.tokSOut)} tok/s)`,
    prefillSpeed: run.tokSPrefill,
    decodeSpeed: run.tokSOut,
    icon: '📊',
    badge: 'LocalMaxxing run',
    vramBandwidth: 'Measured community result',
    description: `${modelName}; ${run.promptTokens || 0} prompt tokens; ${run.outputTokens || 0} output tokens; ${run.contextLength != null ? formatNum(run.contextLength) : 'unknown'} context.`,
