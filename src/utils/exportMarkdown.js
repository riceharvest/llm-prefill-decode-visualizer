...
| Parameter | Value |
| --- | --- |
| Prompt length | ${formatNum(safePrompt)} tok |
| Target output length | ${formatNum(safeOutput)} tok |
| Prefill speed | ${formatNum(prefillSpeed)} tok/s |
| Decode speed (base) | ${formatNum(decodeSpeed)} tok/s |
...
| Initial system prompt | ${formatNum(basePromptTokens)} tok |
| Tool result per turn | +${formatNum(toolOutputTokensPerTurn)} tok |
| Agent thought per turn | ${formatNum(decodeTokensPerTurn)} tok |
| Prefill speed | ${formatNum(prefillSpeed)} tok/s |
| Decode speed | ${formatNum(decodeSpeed)} tok/s |
...
\`\`\`
${cachedTurn ? `
With prefix caching, turn ${cachedTurn.turn}+ only prefills the newly appended tool output (${formatNum(toolOutputTokensPerTurn)} tok) instead of the whole accumulated history:
...
| Final context (KV cache) | ${formatNum(finalContext)} tok |
| Tokens processed (prefill + decode) | ${formatNum(totalTokens)} tok |
